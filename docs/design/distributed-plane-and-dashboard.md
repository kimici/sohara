...
| 阶段 | 内容 | 验收 |
|---|---|---|
| D1 | 单机 Dashboard：admin API 扩展（status/history/approvals/errors/错误环形缓冲/`--admin-token`）+ 内嵌 `/admin/ui`；serve 停止写 history | `sohara serve --admin` 打开 UI：DAG/步骤统计/审批/历史可见，无 token 访问管理端点 401 |
